...
t=300sec
M  thr 90 HV 1010
G1 thr variabile HV variabile file_new
G2 thr 72.2 HV 950 file_new
*/
...
int main (int argc, char** argv)
{
  ...
  //prendo file in input e istanzio i vettori
	...
	string fileInput2  = argv[2];
    vector<float> vec_thr_HV_930;
	vector<float> vec_s_HV_930;
    vector<float> vec_d_HV_930;
	vector<float> vec_t_HV_930;
    vector<float> eff930;

	string fileInput3  = argv[3];
...
if (readData(fileInput1, vec_thr_HV_950, vec_s_HV_950, vec_d_HV_950, vec_t_HV_950) == false)
		  {
		    std::cout << "Problem opening the file: " << fileInput1.c_str() << std::endl;
		    return 1;
		  
		}


if (readData(fileInput2, vec_thr_HV_930, vec_s_HV_930, vec_d_HV_930, vec_t_HV_930) == false)
		  {
		    std::cout << "Problem opening the file: " << fileInput2.c_str() << std::endl;
		    return 1;
		  
		}

if (readData(fileInput3, vec_thr_HV_970, vec_s_HV_970, vec_d_HV_970, vec_t_HV_970) == false)
		  {
		    std::cout << "Problem opening the file: " << fileInput3.c_str() << std::endl;
		    return 1;
		  
		}

	
if (readData(fileInput4, vec_thr_HV_990, vec_s_HV_990, vec_d_HV_990, vec_t_HV_990) == false)
		  {
		    std::cout << "Problem opening the file: " << fileInput4.c_str() << std::endl;
		    return 1;
	...
	for(int i=0;i<vec_thr_HV_930.size();i++) {
    	eff930.push_back(vec_t_HV_930.at(i)/vec_d_HV_930.at(i));
    	cout<<" thr 930 "<< vec_thr_HV_930.at(i)<< "  eff 930  "<< eff930.at	(i)<<endl;
	}
 cout<<"--------------"<<endl;
...
TH2D EFF ("EFF", "EFF", 10, 925, 1025, 8, 55, 135);

     for ( int i = 0; i < 4; i++){
     EFF.Fill(930 ,vec_thr_HV_930.at(i), eff930.at(i));
     }
